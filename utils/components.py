...
from utils.api_client import get_workflow_status


def render_logo(title: str = "Mortgage Dashboard"):
    """Render NatWest logo in sidebar"""
    st.markdown(f"""
    <div class="logo-container">
        ...
        <p style="color: rgba(255, 255, 255, 0.7); font-size: 0.85rem; margin: 0.5rem 0 0 0;">{title}</p>
    </div>
...
def correlate_messages_with_agents(messages: List[Dict], audit_trail: List[Dict]) -> List[Dict]:
    """
    Correlate messages with the agent that created them using audit trail.
    """
    if not audit_trail:
        return messages

    # Build agent execution timeline from audit trail
    agent_timeline = []
    for entry in audit_trail:
        if entry.get('action') == 'autonomous_routing_decision':
            next_agent = entry.get('details', {}).get('next_agent', 'unknown')
    ...
    # Assign agents to messages based on sequence
    enriched_messages = []
    current_agent = "Router"
    agent_index = 0

    for i, msg in enumerate(messages):
        if msg.get('role') == 'human' and i > 0:
            if agent_index < len(agent_timeline):
                current_agent = agent_timeline[agent_index]['agent']
                agent_index += 1
        ...
        enriched_msg['agent'] = agent_display
        enriched_messages.append(enriched_msg)

    return enriched_messages


def _render_stage_display(state: Dict):
    """
    Helper function to render workflow stages (DRY - used by both polling and static views)
    """
    status = state.get('status', 'unknown')
    interrupt_info = state.get('interrupt_info', {})
    current_tool = interrupt_info.get('tool_name', '') if interrupt_info else ''

    # Stage 1: Initial Data Gathering
    if state.get('account_data') and state.get('customer_data'):
        st.success("📊 **1. Initial Data Gathering** ✅")
    else:
        st.info("📊 **1. Initial Data Gathering** - Pending...")

    # Stage 2: Pre-Qualification
    if state.get('pre_qualification'):
        prequal = state['pre_qualification']
        if prequal.get('passed'):
            st.success("✅ **2. Pre-Qualification** ✅ PASSED")
        else:
            st.error(f"❌ **2. Pre-Qualification** ✅ FAILED: {', '.join(prequal.get('reasons', []))}")
    else:
        st.info("✅ **2. Pre-Qualification** - Pending...")

    # Stage 3: Affordability Assessment
    if state.get('affordability'):
        afford = state['affordability']
        if afford.get('passed'):
            st.success("💰 **3. Affordability Assessment** ✅ PASSED")
        else:
            st.error("❌ **3. Affordability Assessment** ✅ FAILED")
    else:
        st.info("💰 **3. Affordability Assessment** - Pending...")

    # Stage 4: Income Verification
    if state.get('income_verification'):
        income_ver = state['income_verification']
        # Check if income review was approved by HITL
        income_review_approved = state.get('income_review_approved', False)

        if income_ver.get('verified'):
            st.success("🔍 **4. Income Verification** ✅ VERIFIED")
        elif income_review_approved:
            # HITL approved despite verification failure
            st.success("🔍 **4. Income Verification** ✅ APPROVED BY REVIEW")
        else:
            # Check if currently awaiting income verification review
            if status == 'awaiting_approval' and current_tool == 'income_verification_review':
                st.warning("⚠️ **4. Income Verification** - AWAITING REVIEW")
            else:
                st.warning("⚠️ **4. Income Verification** ✅ REVIEW REQUIRED")
    else:
        st.info("🔍 **4. Income Verification** - Pending...")

    # Stage 5: Proposal Generation
    if state.get('proposal_draft') or state.get('proposal_text'):
        st.success("📄 **5. Proposal Generation** ✅")
    elif status == 'awaiting_approval' and current_tool == 'finalize_proposal':
        # At compliance review - proposal already generated
        st.success("📄 **5. Proposal Generation** ✅")
    elif status == 'awaiting_approval' and current_tool == 'update_account_details':
        # At system update - proposal already generated
        st.success("📄 **5. Proposal Generation** ✅")
    elif state.get('income_review_approved'):
        # Income review was approved, proposal generation in progress
        st.info("📄 **5. Proposal Generation** - In Progress...")
    else:
        st.info("📄 **5. Proposal Generation** - Pending...")

    # Stage 6: Compliance Review (HITL)
    # CRITICAL: Only treat hitl_approved as compliance approval if we're at or past finalize_proposal
    # If we just approved income verification review, hitl_approved refers to THAT approval, not compliance
    has_proposal = state.get('proposal_draft') or state.get('proposal_text')
    income_review_approved = state.get('income_review_approved', False)

    if status == 'awaiting_approval' and current_tool == 'finalize_proposal':
        # Currently waiting for compliance review approval
        st.warning("⏳ **6. Compliance Review** - AWAITING APPROVAL")
    elif status == 'awaiting_approval' and current_tool == 'update_account_details':
        # At system update stage - compliance already approved
        st.success("🔐 **6. Compliance Review** ✅ APPROVED")
    elif status == 'awaiting_approval' and current_tool == 'income_verification_review':
        # At income verification review - compliance hasn't started yet
        st.info("🔐 **6. Compliance Review** - Pending...")
    elif status == 'in_progress' and income_review_approved and not has_proposal:
        # Just approved income review, workflow generating proposal - compliance hasn't started
        st.info("🔐 **6. Compliance Review** - Pending...")
    elif has_proposal and state.get('hitl_approved') is not None:
        # We have a proposal AND are past income verification review, so hitl_approved refers to compliance
        if state.get('hitl_approved'):
            st.success("🔐 **6. Compliance Review** ✅ APPROVED")
        else:
            st.error("❌ **6. Compliance Review** ✅ REJECTED")
    else:
        st.info("🔐 **6. Compliance Review** - Pending...")

    # Stage 7: Customer Approval
    if state.get('customer_approved') is not None:
        if state.get('customer_approved'):
            st.success("👤 **7. Customer Approval** ✅ ACCEPTED")
        else:
            st.error("❌ **7. Customer Approval** ✅ DECLINED")
    elif status == 'awaiting_customer_approval':
        st.warning("⏳ **7. Customer Approval** - AWAITING DECISION")
    elif status == 'awaiting_approval' and current_tool == 'update_account_details':
        # At system update stage - customer already approved
        st.success("👤 **7. Customer Approval** ✅ ACCEPTED")
    else:
        st.info("👤 **7. Customer Approval** - Pending...")

    # Stage 8: System Update
    if state.get('system_update_confirmation'):
        st.success("⚙️ **8. System Update** ✅ COMPLETED")
    elif status == 'awaiting_approval' and current_tool == 'update_account_details':
        # Waiting for system update approval
        st.warning("⏳ **8. System Update** - AWAITING APPROVAL")
    else:
        st.info("⚙️ **8. System Update** - Pending...")


def render_workflow_stages(status_data: Dict, show_agent_details: bool = True):
    """
    Render workflow stages statically (no polling)

    Args:
        status_data: Workflow state data
        show_agent_details: If True, shows full agent reasoning (employee view).
                           If False, shows simplified progress (customer view).
    """
    # Calculate progress based on completed stages
    completed_steps = 0
    total_steps = 8

    if status_data.get('account_data') and status_data.get('customer_data'):
        completed_steps += 1
    if status_data.get('pre_qualification'):
        completed_steps += 1
    if status_data.get('affordability'):
        completed_steps += 1
    if status_data.get('income_verification'):
        completed_steps += 1
    if status_data.get('proposal_draft') or status_data.get('proposal_text'):
        completed_steps += 1
    if status_data.get('hitl_approved') is not None:
        completed_steps += 1
    if status_data.get('customer_approved') is not None:
        completed_steps += 1
    if status_data.get('system_update_confirmation'):
        completed_steps += 1

    # Show progress bar
    progress = completed_steps / total_steps
    st.progress(progress, text=f"Progress: {completed_steps}/{total_steps} steps complete")

    st.markdown("#### 📋 Workflow Progress")

    # Render stages using shared display logic
    _render_stage_display(status_data)

    # Show agent reasoning only for employee view
    if show_agent_details and status_data.get('messages'):
        st.markdown("---")
        with st.expander("🤖 Agent Reasoning (Click to expand)", expanded=False):
            messages = status_data['messages']
            audit_trail = status_data.get('audit_trail', [])
            enriched_messages = correlate_messages_with_agents(messages, audit_trail)

            st.markdown(f"**Total Messages: {len(enriched_messages)}**")
            st.markdown("---")
            ...
            for i, msg in enumerate(enriched_messages):
                with st.container():
                    if isinstance(msg, dict):
                        ...
                        if role == 'ai':
                            st.markdown(f"**Message {i+1} [{icon} {agent} - OUTPUT]**")
                        elif role == 'tool':
                            st.markdown(f"**Message {i+1} [🔧 TOOL RESPONSE]**")
                        elif role == 'human':
                            st.markdown(f"**Message {i+1} [{icon} {agent} - PROMPT]**")
                        else:
                            st.markdown(f"**Message {i+1} [{role.upper()}]**")

                        if content and len(content) > 0:
                            if len(content) > 500:
                                with st.expander(f"View content ({len(content)} chars)", expanded=False):
                            ...
                            if tool_calls or additional_kwargs.get('tool_calls'):
                                st.success("🔧 **AI Agent Tool Invocation**")
                                calls = tool_calls or additional_kwargs.get('tool_calls', [])

                                if calls:
                                    st.markdown(f"**Invoking {len(calls)} tool(s):**")
                                    for idx, call in enumerate(calls):
                                        if isinstance(call, dict):
                                            ...
                                            if tool_args:
                                                with st.expander(f"View arguments", expanded=False):
                                                    st.json(tool_args)
                            else:
                                st.info("_(Empty message)_")

                    st.markdown("---")


def poll_workflow_progress(workflow_id: str, max_polls: int = 120, show_details: bool = True):
    """
    Poll workflow and display real-time progress.

    Args:
        workflow_id: ID of the workflow to poll
        max_polls: Maximum number of polling iterations
        show_details: If True, shows detailed agent reasoning (employee view).
                     If False, shows simplified progress (customer view).
    ...
    graph_container = st.empty()  # NEW: Container for dynamic graph
    ...
    for poll_count in range(max_polls):
        state = get_workflow_status(workflow_id)

        if not state:
            status_container.error("❌ Unable to fetch workflow status")
            break
        ...
        progress_bar.progress(progress, text=f"Progress: {completed_count}/{total_steps} steps complete")

        # Calculate actual elapsed time from workflow creation
        from datetime import datetime
        created_at = state.get('created_at')
        elapsed_display = f"{poll_count}s"  # Fallback to poll count
        if created_at:
            try:
                ...
                if elapsed_seconds >= 60:
                    ...
                    elapsed_display = f"{elapsed_minutes}m {elapsed_secs}s"
                else:
        ...
        with status_container.container():
            if status == 'in_progress':
                st.info(f"🔄 **Processing** - Please wait... ({elapsed_display} elapsed)")
            elif status == 'awaiting_approval':
                st.warning("⏸️ **Under Review** - Your request is being reviewed")
            elif status == 'awaiting_customer_approval':
                st.warning("⏸️ **Customer Decision Pending** - Waiting for customer approval")
            elif status == 'completed':
                st.success("✅ **Completed**")
            elif status == 'failed':
                st.error("❌ **Failed**")

        # NEW: Show workflow execution graph (updates every poll)
        with graph_container.container():
            from utils.graph_visualization import create_agent_workflow_graph

            agent_stats = state.get('agent_execution_stats', {})

            if agent_stats:
                with st.expander("📊 Workflow Execution Graph", expanded=True):
                    st.markdown("**Visual representation of agent execution flow**")

                    # Render graph with unique key for each poll iteration
                    fig = create_agent_workflow_graph(agent_stats)
                    st.plotly_chart(fig, width='stretch',
                                   key=f"agent_workflow_graph_poll_{poll_count}")

                    # Add spacing between graph and current stage
                    st.markdown("<br>", unsafe_allow_html=True)

                    # Show current stage below graph
                    current_stage = state.get('current_stage', {})
                    if current_stage:
                        icon = current_stage.get('icon', '⚙️')
                        stage_name = current_stage.get('stage_name', 'Processing')

                        col1, col2 = st.columns([3, 1])
                        with col1:
                            st.info(f"**{icon} Current Stage:** {stage_name}")
                        with col2:
                            st.metric("⏱️ Time Elapsed", elapsed_display)
        ...
        # Show workflow stages - ALL STAGES DISPLAYED UPFRONT
        with stages_container.container():
            st.markdown("#### 📋 Workflow Progress")

            # Update completion tracking
            if state.get('account_data') and state.get('customer_data'):
                completed_steps['data_gathering'] = True
            if state.get('pre_qualification'):
                completed_steps['pre_qualification'] = True
            if state.get('affordability'):
                completed_steps['affordability'] = True
            if state.get('income_verification'):
                completed_steps['income_verification'] = True
            if state.get('proposal_draft') or state.get('proposal_text'):
                completed_steps['proposal_generation'] = True
            if state.get('hitl_approved') is not None:
                completed_steps['compliance_review'] = True
            if state.get('customer_approved') is not None:
                completed_steps['customer_approval'] = True
            if state.get('system_update_confirmation'):
                completed_steps['system_update'] = True

            # Render stages using shared display logic
            _render_stage_display(state)

        # Show agent reasoning for employee view (if show_details=True)
        if show_details and state.get('messages'):
        ...
        if status == 'completed':
            progress_bar.progress(1.0, text=f"Progress: {total_steps}/{total_steps} steps complete ✅")
            return state
    ...
    st.warning(f"⚠️ Still processing... Please check back later.")
    return state
